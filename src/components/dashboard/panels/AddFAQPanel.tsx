...
import { getAddFAQPanel, toggleAddFAQPanel } from "@JCKConsultant/redux/reducers/panelsSlice"
import WYSIWYGEditor from "@JCKConsultant/components/misc/WYSIWYGEditor"
import { useMutation } from "react-query"
import { CreateFAQ, FetchFAQs } from "@JCKConsultant/services/settings/settings.apis"
...
export default function AddFAQPanel() {
	const addFAQPanel = useSelector(getAddFAQPanel)
	const { show, data, params } = addFAQPanel
	const dispatcher = useDispatch()

	const _closePanel = () => {
		dispatcher(toggleAddFAQPanel({ status: false }))
	}
	...
	const createFAQApi = useMutation(CreateFAQ, {
		onSuccess(res: any) {
			if (res?.status) {
				_closePanel()
								...
								<Dialog.Panel className="pointer-events-auto relative w-screen max-w-md">
									...
									<div className="flex h-full flex-col overflow-y-scroll bg-white py-6 shadow-xl">
										<div className="px-4 sm:px-6">
											<Dialog.Title className="text-base font-semibold leading-6 text-gray-900">Create New FAQ</Dialog.Title>
										</div>
										<hr className="my-8" />
										...
										<div className="relative mt-6 flex-1 px-4 sm:px-6">
											...
											<form onSubmit={_handleSubmit}>
												...
												<div className="relative mb-12">
													...
													<WYSIWYGEditor inputName="content" />
												</div>

												<div className="">
													<button disabled={isCreating} type="submit" className="p-3 mt-5 rounded-lg text-white bg-blue disabled:bg-blue/50">
														{!isCreating && "Create FAQ"}
														{isCreating && <Spinner />}
													</button>
												</div>
											</form>
